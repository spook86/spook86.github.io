...
var preProcessUrl = function preProcessUrl(id, url) {
	if (url.startsWith('https://') || url.startsWith('http://') || url.startsWith('/')) {
		return url;
	}
	var PostAsset = hexo.model('PostAsset');
	var asset = PostAsset.findOne({ post: id, slug: url });
...
hexo.extend.tag.register('aplayer', function (args) {
	var _args = _slicedToArray(args, 3),
	    ...
	    lrcOpt = false,
	    width = '',
	    pic = args[3] && args[3] !== 'narrow' && args[3] !== 'autoplay' && !args[3].includes('lrc:') && !args[3].includes('width:') ? preProcessUrl(this._id, args[3]) : '',
	    id = 'aplayer' + counter++,
	    raw = '',
	    content = '';
...
hexo.extend.tag.register('aplayerlist', function (args, content) {
	...
	try {
		...
		resultOptions.music.forEach(function (info) {
			info.url = preProcessUrl(_this._id, info.url);
			info.pic = info.pic ? preProcessUrl(_this._id, info.pic) : '';
		});
